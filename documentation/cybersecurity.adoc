...
:attack-path: user-attack-path.svg
...
Using gds.graph.create.cypher method, we will create a graph projection using cypher query for both node and relationship specification. 
For finding attack paths, we have to cover all the nodes and relationships to be included for GDS procedures.
So instead of specifying all node labels and relationship types to create graph, we specify cypher queries to include all.
[source,cypher]
----
// Build projection
...
Now we try to figure out the possible attack paths. For this, we take a user in question and check what all high value objects (or crown jewels) this user can reach directly or indirectly?
...
WITH u
// Targetting a high value object (we call it with name "crown jewel")
...
Now, lets check what are those assets and what are those possible attack paths?
...
Now, lets check how "Piedad Flatley" can reach upto ENTERPRISE DOMAIN CONTROLLERS group?
...
WITH u
// Targetting a high value object (we call it with name "crown jewel")
MATCH (crownJewel {highvalue:true, objectid: "TestCompany.Local-S-1-5-9"})
WITH u, id(crownJewel) AS crownJewelId
// Get shortest paths from the user to the crown jewel object
CALL gds.shortestPath.dijkstra.stream("attackPaths", {
    sourceNode: id(u), 
...
So now we see that the user "Piedad Flatley" is a member of "Domain Admins" group which is an admin for computer "FLLABDC@TestCompany.Loca". And this computer is a member of "ENTERPRISE DOMAIN CONTROLLERS" group.
...
We saw possible attack paths for one user, what is the possible extent of this? We can check all the possible paths like this in whole network.
[source,cypher]
----
