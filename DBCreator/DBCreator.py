...
class MainMenu(cmd.Cmd):
    ...
    def help_setnodes(self):
        print("Set base number of nodes to generate - (Minimum 300) (default 300)")

    def help_setdomain(self):
        print("Set domain name (default 'TestCompany.Local')")
    ...
    def do_setnodes(self, args):
        ...
        nodesValue = self.num_nodes
        
        if passed != "":
            try:                
                nodesValue = int(passed)
            except ValueError:
                pass
                
        nodesValue = int(self.m.input_default(
            "Set base number of nodes to generate - (Minimum 300)", self.num_nodes))
            
        if (nodesValue < 300):
            print("Please supply a node count more than 300.")
            return
        else:
            self.num_nodes = nodesValue
            return
    ...
    def do_setdomain(self, args):
        ...
        self.domain = self.m.input_default("Domain", self.domain)
        print("")
        print("New Settings:")
    ...
    def generate_data(self):
        ...
        session.run(
            'MERGE (n:GPO {name:$gpo}) MERGE (m:Domain {name:$domain}) MERGE (n)-[:GP_LINK {isacl:false, enforced:toBoolean(false)}]->(m)', gpo=gpo_name, domain=self.domain)
        session.run(
        ...
        session.run(
            'MERGE (n:GPO {name:"DEFAULT DOMAIN CONTROLLERS POLICY@$domain"}) MERGE (m:OU {objectid:$guid}) MERGE (n)-[:GP_LINK {isacl:false, enforced:toBoolean(false)}]->(m)', domain=self.domain, guid=dcou)

        # Ent Admins -> Domain Node
        group_name = "ENTERPRISE ADMINS@{}".format(self.domain)
        session.run(
            'MERGE (n:Domain {name:$domain}) MERGE (m:Group {name:$gname}) MERGE (m)-[:GENERIC_ALL {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        ...
        session.run(
            'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WRITE_OWNER {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        session.run(
            'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:WRITE_DACL {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        session.run(
            'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:DC_SYNC {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        session.run(
            'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GET_CHANGES {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        session.run(
            'MERGE (n:Domain {name:$domain}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:GET_CHANGES_ALL {isacl:true}]->(n)', domain=self.domain, gname=group_name)
        ...
        for i in range(1, self.num_nodes+1):
            ...
            if (len(props) > 500):
                session.run(
                    'UNWIND $props as prop MERGE (n:Base {objectid: prop.id}) SET n:Computer, n += prop.props WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', props=props, gname=group_name)
                props = []
        session.run(
            'UNWIND $props as prop MERGE (n:Base {objectid:prop.id}) SET n:Computer, n += prop.props WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', props=props, gname=group_name)
        ...
        for state in states:
            ...
            sid = cs(ridcount)
            session.run(
                'MERGE (n:Base {objectid:$sid}) SET n:Computer,n.name=$name WITH n MATCH (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', sid=sid, name=comp_name, gname=group_name)
            session.run(
                'MATCH (n:Computer {objectid:$sid}) WITH n MATCH (m:OU {objectid:$dcou}) WITH n,m MERGE (m)-[:CONTAINS]->(n)', sid=sid, dcou=dcou)
            session.run(
                'MATCH (n:Computer {objectid:$sid}) WITH n MATCH (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', sid=sid, gname=cn("ENTERPRISE DOMAIN CONTROLLERS"))
            session.run(
                'MERGE (n:Computer {objectid:$sid}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (m)-[:ADMIN_TO]->(n)', sid=sid, gname=cn("DOMAIN ADMINS"))
        ...
        for i in range(1, self.num_nodes+1):
            ...
            user_name = "{}{}{:02d}@{}".format(
                first, last, i, self.domain)
            user_name = user_name.format(first, last, i)
            ...
            if (len(props) > 500):
                session.run(
                    'UNWIND $props as prop MERGE (n:Base {objectid:prop.id}) SET n:User, n += prop.props WITH n MATCH (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', props=props, gname=group_name)
                props = []
        ...
        session.run(
            'MATCH (n:Computer) MATCH (m:Group {objectid: $id}) MERGE (m)-[:ADMIN_TO]->(n)', id=cs(512))
        ...
        for da in das:
            session.run(
                'MERGE (n:User {name:$name}) WITH n MERGE (m:Group {name:$gname}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', name=da, gname=cn("DOMAIN ADMINS"))
        ...
        for group in groups:
            ...
            if (len(props) > 500):
                session.run(
                    'UNWIND $props AS prop MERGE (n:Group {name:prop.a}) WITH n,prop MERGE (m:Group {name:prop.b}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', props=props)
                props = []
        ...
        for user in users:
            ...
            if len(props) > 500:
                session.run(
                    'UNWIND $props AS prop MERGE (n:User {name:prop.a}) WITH n,prop MERGE (m:Group {name:prop.b}) WITH n,m MERGE (n)-[:MEMBER_OF]->(m)', props=props)
                props = []
        ...
        for x in range(0, total_it_groups):
            ...
            if len(props) > 500:
                session.run(
                    'UNWIND $props AS prop MERGE (n:Group {name:prop.a}) WITH n,prop MERGE (m:Computer {name:prop.b}) WITH n,m MERGE (n)-[:ADMIN_TO]->(m)', props=props)
                props = []
        ...
        print("Adding RDP/EXECUTE_DCOM/ALLOWED_TO_DELEGATETo")
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:User {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:CAN_RDP]->(m)', props=props)
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:User {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:EXECUTE_DCOM]->(m)', props=props)
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:Group {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:CAN_RDP]->(m)', props=props)
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:Group {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:EXECUTE_DCOM]->(m)', props=props)
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:User {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:ALLOWED_TO_DELEGATE]->(m)', props=props)
        ...
        session.run(
            'UNWIND $props AS prop MERGE (n:Computer {name: prop.a}) MERGE (m:Computer {name: prop.b}) MERGE (n)-[r:ALLOWED_TO_DELEGATE]->(m)', props=props)
        ...
        for user in users:
            ...
            if (len(props) > 500):
                session.run(
                    'UNWIND $props AS prop MERGE (n:User {name:prop.a}) WITH n,prop MERGE (m:Computer {name:prop.b}) WITH n,m MERGE (m)-[:HAS_SESSION]->(n)', props=props)
                props = []
        ...
        for x in computers:
            ...
            if len(props) > 500:
                session.run(
                    'UNWIND $props as prop MATCH (n:Computer {name:prop.name}) WITH n MATCH (m:Group {name:$gname}) WITH m,n MERGE (m)-[r:GENERIC_ALL {isacl:true}]->(n)', props=props, gname=group_name)
                props = []
        ...
        for x in users:
            ...
            if len(props) > 500:
                session.run(
                    'UNWIND $props as prop MATCH (n:User {name:prop.name}) WITH n MATCH (m:Group {name:$gname}) WITH m,n MERGE (m)-[r:GENERIC_ALL {isacl:true}]->(n)', props=props, gname=group_name)
                props = []
        ...
        for x in groups:
            ...
            if len(props) > 500:
                session.run(
                    'UNWIND $props as prop MATCH (n:Group {name:prop.name}) WITH n MATCH (m:Group {name:$gname}) WITH m,n MERGE (m)-[r:GENERIC_ALL {isacl:true}]->(n)', props=props, gname=group_name)
                props = []
        ...
        for g in gpos:
            ...
            for l in linked_ous:
                guid = ou_guid_map[l]
                session.run(
                    "MERGE (n:GPO {name:$gponame}) WITH n MERGE (m:OU {objectid:$guid}) WITH n,m MERGE (n)-[r:GP_LINK]->(m)", gponame=g, guid=guid)
        ...
        for l in linked_ous:
            guid = ou_guid_map[l]
            session.run(
                "MERGE (n:Domain {name:$gponame}) WITH n MERGE (m:OU {objectid:$guid}) WITH n,m MERGE (n)-[r:GP_LINK]->(m)", gponame=self.domain, guid=guid)

        gpos.append("DEFAULT DOMAIN POLICY@{}".format(self.domain))
        gpos.append("DEFAULT DOMAIN CONTROLLER POLICY@{}".format(self.domain))

        acl_list = ["GENERIC_ALL"] * 10 + ["GENERIC_WRITE"] * 15 + ["WRITE_OWNER"] * 15 + ["WRITE_DACL"] * \
            15 + ["ADD_MEMBER"] * 30 + ["FORCE_CHANGE_PASSWORD"] * \
            15 + ["READ_LAPS_PASSWORD"] * 10
        ...
        for i in acl_groups:
            ...
            if ace == "GENERIC_ALL" or ace == 'GENERIC_WRITE' or ace == 'WRITE_OWNER' or ace == 'WRITE_DACL':
            ...
            elif ace == 'READ_LAPS_PASSWORD':
                p = random.choice(all_principals)
                targ = random.choice(computers)
                session.run(
                    'MERGE (n {name:$principal}) MERGE (m:Computer {name:$target}) MERGE (n)-[r:READ_LAPS_PASSWORD]->(m)', target=targ, principal=p)
            else:
