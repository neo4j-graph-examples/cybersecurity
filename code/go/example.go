...
func runQuery(uri, database, username, password string) (_ []string, err error) {
	...
	query := "	MATCH (u:User {name: $name})-[:CAN_RDP]->(r) RETURN r.name as computer
	params := map[string]any{"name": "HilaryOlivia226@TestCompany.Local"}
	...
	computers := make([]string, len(result.Records))
	for i, record := range result.Records {
		// this assumes all actors have names, hence ignoring the 2nd returned value
		name, _, err := neo4j.GetRecordValue[string](record, "computer")
		if err != nil {
			return nil, err
		}
		computers[i] = name
